...
fun RVfromR0V0(R0: MultiArray<Double, D1>, V0: MultiArray<Double, D1>, t: Double, mu: Double): Pair<MultiArray<Double, D1>, MultiArray<Double, D1>> {
  ...
  val (f, g) = fg(x, t, r0, alpha, mu)
  val R = f * R0 + g * V0
  val r = R.norm()
  val (fDot, gDot) = fDotgDot(x, r, r0, alpha, mu)
  val V = fDot * R0 + gDot * V0
  return R to V
}

fun RVfromR0V0TA(R0: MultiArray<Double, D1>, V0: MultiArray<Double, D1>, dta: Double, mu: Double): Pair<MultiArray<Double, D1>, MultiArray<Double, D1>> {
  val (f, g) = fgTA(R0, V0, dta, mu)
  val (fDot, gDot) = fDotgDotTA(R0, V0, dta, mu)
  val R = f * R0 + g * V0
  val V = fDot * R0 + gDot * V0
  return R to V
}
